...
AC_DEFUN([WONDERCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build wondercoin-qt GUI (default=auto)])],
    [
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); wondercoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   WONDERCOIN_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _WONDERCOIN_QT_FIND_LIBS
...
AC_DEFUN([WONDERCOIN_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  WONDERCOIN_QT_CHECK([_WONDERCOIN_QT_FIND_LIBS])
  ...
  dnl the final binary as well. _WONDERCOIN_QT_CHECK_STATIC_PLUGIN does
  ...
  if test "x$wondercoin_cv_static_qt" = xyes; then
    _WONDERCOIN_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _WONDERCOIN_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _WONDERCOIN_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      ...
      _WONDERCOIN_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _WONDERCOIN_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _WONDERCOIN_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  WONDERCOIN_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  WONDERCOIN_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  WONDERCOIN_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  WONDERCOIN_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  WONDERCOIN_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  WONDERCOIN_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  if test x$wondercoin_enable_qt = xyes; then
    AC_MSG_RESULT([$wondercoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$wondercoin_enable_qt])
  fi
...
dnl
dnl _WONDERCOIN_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_WONDERCOIN_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); WONDERCOIN_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _WONDERCOIN_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_WONDERCOIN_QT_CHECK_STATIC_LIBS], [
...
dnl
dnl _WONDERCOIN_QT_FIND_LIBS
dnl ---------------------
dnl
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_WONDERCOIN_QT_FIND_LIBS],[
  WONDERCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [WONDERCOIN_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  WONDERCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [WONDERCOIN_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  WONDERCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [WONDERCOIN_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  WONDERCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [WONDERCOIN_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
