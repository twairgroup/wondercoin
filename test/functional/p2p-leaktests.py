...
Also test that nodes that send unsupported service bits to wondercoind are disconnected
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if wondercoind ban behavior changes
    def on_open(self, conn):
        super().on_open(conn)
...
class P2PLeakTest(WondercoinTestFramework):
