...
#include <qt/wondercoinaddressvalidator.h>
...
QString formatWondercoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("wondercoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(WondercoinUnits::format(WondercoinUnits::WDC, info.amount, false, WondercoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", WONDERCOIN_CONF_FILENAME));
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("wondercoin-%s.desktop", chain);
}
