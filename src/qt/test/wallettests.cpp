...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<WondercoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_wondercoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_wondercoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_wondercoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = WondercoinUnits::formatWithUnit(unit, balance, false, WondercoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    WondercoinAmountField* amountInput = receiveCoinsDialog.findChild<WondercoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("wondercoin:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_wondercoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
