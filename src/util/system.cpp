...
const char * const WONDERCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/wondercoin/wondercoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //wondercoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", WONDERCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Wondercoin
    // macOS: ~/Library/Application Support/Wondercoin
    // Unix-like: ~/.wondercoin
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", WONDERCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Wondercoin Core copyright is not removed by accident
    if (copyright_devs.find("Wondercoin Core") == std::string::npos) {
