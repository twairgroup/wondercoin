...
static void SetupWondercoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " wondercoin-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  wondercoin-tx [options] <hex-tx> [commands]  Update hex-encoded wondercoin transaction\n"
                "or:     wondercoin-tx [options] -create [commands]   Create hex-encoded wondercoin transaction\n"
                "\n";
