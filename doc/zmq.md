...
The Wondercoin Core daemon can be configured to act as a trusted "border
router", implementing the wondercoin wire protocol and relay, making
...
The ZeroMQ feature in Wondercoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building wondercoind:
...
For instance:
    ...
    $ wondercoind -zmqpubhashtx=tcp://127.0.0.1:28666 \
               -zmqpubhashtx=tcp://192.168.1.2:28666 \
               -zmqpubrawtx=ipc:///tmp/wondercoind.tx.raw \
...
These options can also be provided in wondercoin.conf.
...
From the perspective of wondercoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into wondercoind directly. Furthermore, no information is
...
using. Wondercoind appends an up-counting sequence number to each
