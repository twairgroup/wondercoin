...
*Setup instructions for a Gitian build of Wondercoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Wondercoin
...
- [Building Wondercoin Core](#building-wondercoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-wondercoin-host/README.md).
...
Clone the git repositories for wondercoin and Gitian.
...
in the wondercoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Wondercoin Core
----------------

To build Wondercoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the wondercoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/wondercoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/wondercoin/wondercoin
...
For example:
```bash
URL=https://github.com/laanwj/wondercoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit wondercoin=${COMMIT} --url wondercoin=${URL} ../wondercoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit wondercoin=${COMMIT} --url wondercoin=${URL} ../wondercoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit wondercoin=${COMMIT} --url wondercoin=${URL} ../wondercoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the wondercoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../wondercoin/contrib/gitian-descriptors/*|sort|uniq )
...
WDCPATH=/some/root/path/wondercoin
SIGPATH=/some/root/path/wondercoin-detached-sigs

./bin/gbuild --url wondercoin=${WDCPATH},signature=${SIGPATH} ../wondercoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/wondercoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/wondercoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/wondercoin-osx-build.assert
```
...
[wondercoin-core/gitian.sigs](https://github.com/wondercoin-core/gitian.sigs/) repository, or if that's not possible create a pull
