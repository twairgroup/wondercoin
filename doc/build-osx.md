...
This guide describes how to build wondercoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Wondercoin Core from source.
...
### 4. Clone Wondercoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Wondercoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `wondercoind` or  `wondercoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/wondercoin/wondercoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Wondercoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/wondercoin/wondercoin/issues/7714).
...
You can deploy a `.dmg` containing the Wondercoin Core application using `make deploy`.
...
There are many ways to configure Wondercoin Core, here are a few common examples:
...
Run the following in your terminal to compile Wondercoin Core:
...
## Running Wondercoin Core

Wondercoin Core should now be available at `./src/wondercoind`.
If you compiled support for the GUI, it should be available at `./src/qt/wondercoin-qt`.

The first time you run `wondercoind` or `wondercoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Wondercoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Wondercoin"

touch "/Users/${USER}/Library/Application Support/Wondercoin/wondercoin.conf"
...
./src/qt/wondercoin-qt -server # Starts the wondercoin-qt server mode, allows wondercoin-cli control
```
