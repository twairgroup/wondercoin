...
###   wondercoincore.org and wondercoin.org and compares them.
...
WORKINGDIR="/tmp/wondercoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://wondercoincore.org"
HOST2="https://wondercoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="wondercoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'wondercoin-',
...
if [ $? -ne 0 ]; then
   echo "wondercoin.org failed to provide signature file, but wondercoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "wondercoin.org and wondercoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Wondercoin Core binary release signing key installed?"
   fi
