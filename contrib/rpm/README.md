...
The RPM spec file provided here is for Wondercoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://wondercoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Wondercoin git repository](https://github.com/wondercoin/wondercoin) but are not part of
...
Currently only `Source10` is used, for the example `wondercoin.conf` file.
...
in the wondercoin git repository. At present only an SVG version of the Wondercoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the wondercoin
...
needed when 0.12.1 is released, a proper fix is already in the Wondercoin git
master branch.
...
idea to build Wondercoin Core against a static version of that library compiled
...
The `wondercoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `wondercoin-server` package. At this point, most RPM based
...
The `wondercoin-server` package should have SELinux support. How to properly do
...
Wondercoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/wondercoin/). He has been
packaging Wondercoin for Fedora at least since 2012.
...
4. I renamed the `wondercoin` package that contains the Qt GUI to `wondercoin-core` as
that appears to be how the general population refers to it, in contrast to
`wondercoin-xt` or `wondercoin-classic`. I wanted to make sure the general population
